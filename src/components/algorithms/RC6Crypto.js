    ...
    const handleOperation = async (operation) => {
        try {
            ...
            const response = operation === 'encrypt'
                ? await rc6Encrypt(key, plaintext, encoding, rounds)
                : await rc6Decrypt(key, ciphertext, encoding, rounds);
        ...
        <Box>
            ...
            <Paper sx={{ p: 3, mb: 4 }}>
                ...
                <Grid container spacing={3}>
                    ...
                    <Grid item xs={12} md={6}>
                        <FormControl fullWidth sx={{ mb: 2 }}>
                            <FormLabel component="legend" sx={{ mb: 1, display: 'flex', alignItems: 'center' }}>
                                <TuneIcon sx={{ mr: 1, fontSize: '1rem' }} /> 加密轮数: {rounds}
                            </FormLabel>
                            <Slider
                                value={rounds}
                ...
                <Typography variant="caption" color="text.secondary">
                    RC6支持可变长度密钥，最大32字节。密钥长度越长，安全性越高
                </Typography>
            </Paper>
